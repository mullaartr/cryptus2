...
@Component
public class VerkoopTransactieDaoJdbc implements TransactionDao{

    private final Logger logger =
            LoggerFactory.getLogger(VerkoopTransactieDaoJdbc.class);
    private final JdbcTemplate jdbcTemplate;


    @Autowired
    public VerkoopTransactieDaoJdbc(JdbcTemplate jdbcTemplate) {
        super();
        this.jdbcTemplate = jdbcTemplate;
        logger.info("Nieuwe verkooptransactionDaoJdbc");
    }
    ...
    private PreparedStatement insertTransactionStatement(Transaction transaction, Connection connection) throws SQLException {
        try {
            PreparedStatement ps = connection.prepareStatement("INSERT INTO " +
                            ...
                            "creditiban, euroammount, creditPortefeuilleID," +
                            "creditassetId, " +
                            "assetammount) " +
                            "VALUES (?,?,?,?,?,?,?) ",
    ...
    public List<Transaction> findTransactions() {
        String sql = "SELECT * FROM transactie where debitIban is null and debitportefeuilleID is null and debitassetId is null; ";
        List<Transaction> transactions = new ArrayList<>();
        try {
    ...
    public Optional<Transaction> findTransactionById(int transactionId) {
        String sql = "SELECT * FROM transactie where transactieId = ? and debitassetId is null and debitportefeuilleID is null and debitassetId is null; ";
